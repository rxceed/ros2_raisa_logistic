...
class humanTrackNode : public rclcpp::Node
{
    ...
    humanTrackNode() : Node("human_tracking")
    {
        ...
        auto hip_callback = [this](std_msgs::msg::Bool::UniquePtr msg)->void
        {
            hipVisible = msg->data;
        };
        subscriber_ = this->create_subscription<std_msgs::msg::Int32>("/vision/human_zone", 10, callback_sub);
        hip_subscriber = this->create_subscription<std_msgs::msg::Bool>("/vision/hip_visible", 10, hip_callback);
        auto callback = [this]()->void
        {
            ...
            if(hipVisible && (humanState == 1 || humanState == 2))
            {
                req_postSpeed->set__vx(0.15);
            }
            else
            {
    ...
    rclcpp::Subscription<std_msgs::msg::Bool>::SharedPtr hip_subscriber;
